...
const COMMAND_MENU_PLACEHOLDER = 'جستجو یا اجرای دستور...'
...
describe('SearchProvider and CommandMenu', () => {
  ...
  it('renders the command palette when the palette is open', async () => {
    ...
    await expect
      ...
      .toBeInTheDocument()
    await expect.element(getByText('پوسته')).toBeInTheDocument()
    await expect.element(getByText('روشن')).toBeInTheDocument()
    await expect.element(getByText('تیره')).toBeInTheDocument()
    await expect.element(getByText('سیستم')).toBeInTheDocument()
    await expect.element(getByText('نمای کلی')).toBeInTheDocument()
  })
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    await userEvent.click(screen.getByText('پایش‌ها'))

    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/monitors' })
    await expect
      .element(screen.getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })

  it('navigates to another workbench route', async () => {
    const screen = await renderWithSearchProvider()
    const { getByPlaceholder, getByRole } = screen

    await openCommandPalette(screen)

    await userEvent.click(getByRole('option', { name: 'تنظیمات' }))
  ...
  it('applies theme and closes the palette when a theme command is chosen', async () => {
    ...
    await userEvent.click(screen.getByText('تیره'))
  ...
  it('shows empty state when the filter matches nothing', async () => {
    ...
    await expect
      .element(screen.getByText('نتیجه‌ای پیدا نشد.'))
      .toBeInTheDocument()
  })
})
